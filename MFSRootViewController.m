	...
	if (!_specifiers)
	{
		...
		[[LSApplicationWorkspace defaultWorkspace] enumerateApplicationsOfType:0 block:^(LSApplicationProxy* appProxy)
		{
			...
			if (version && bundleIdentifier) {
				...
				[appSpecifier setProperty:[NSString stringWithFormat:@"Identifier: %@", bundleIdentifier] forKey:@"subTitle"];
			} else if (version) {
				[appSpecifier setProperty:[NSString stringWithFormat:@"Version: %@", version] forKey:@"subTitle"];
			}

			UIImage *appIcon = nil;
			if (bundleIdentifier && [UIImage respondsToSelector:@selector(_applicationIconImageForBundleIdentifier:format:scale:)]) {
				appIcon = [UIImage _applicationIconImageForBundleIdentifier:bundleIdentifier format:0 scale:[UIScreen mainScreen].scale];
			}

			if (!appIcon) {
				if (@available(iOS 13.0, *)) {
					appIcon = [UIImage systemImageNamed:@"app.window.checkmark"];
				}
			}

			if (appIcon) {
				[appSpecifier setProperty:appIcon forKey:@"iconImage"];
			}
